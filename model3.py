...
def model(X1, X2, X3,gt, para_lambda, dims, act, lr, epochs, batch_size):
    """
    Building model
    :rtype: object
    ...
    net_ae3 = Net_ae(3, dims[2], para_lambda, act[1])

    net_dg1 = Net_dg(1, dims[3], act[2])
    net_dg2 = Net_dg(2, dims[4], act[3])
    net_dg3 = Net_dg(3, dims[5], act[3])


    H = np.random.uniform(0, 1, [X1.shape[0], dims[3][0]])
    x1_input = tf.placeholder(np.float32, [None, dims[0][0]])
    x2_input = tf.placeholder(np.float32, [None, dims[1][0]])
    x3_input = tf.placeholder(np.float32, [None, dims[2][0]])

    with tf.variable_scope("H"):
        h_input = tf.Variable(xavier_init(batch_size, dims[3][0]), name='LatentSpaceData')
    ...
    fea3_latent = tf.placeholder(np.float32, [None, dims[2][-1]])
 

    loss_pre = net_ae1.loss_reconstruct(x1_input) + net_ae2.loss_reconstruct(x2_input) + net_ae3.loss_reconstruct(x3_input)
    pre_train = tf.train.AdamOptimizer(lr[0]).minimize(loss_pre)

    loss_ae = net_ae1.loss_total(x1_input, fea1_latent) + net_ae2.loss_total(x2_input, fea2_latent)+ net_ae3.loss_total(x3_input, fea3_latent)
    update_ae = tf.train.AdamOptimizer(lr[1]).minimize(loss_ae, var_list=[net_ae1.netpara,net_ae2.netpara,net_ae3.netpara])
    z_half1 = net_ae1.get_z_half(x1_input)
    z_half2 = net_ae2.get_z_half(x2_input)
    z_half3 = net_ae3.get_z_half(x3_input)

    loss_dg = para_lambda * (
                net_dg1.loss_degradation(h_input, fea1_latent) + net_dg2.loss_degradation(h_input, fea2_latent)+ net_dg3.loss_degradation(h_input, fea3_latent))
    update_dg = tf.train.AdamOptimizer(lr[2]).minimize(loss_dg, var_list=[net_dg1.netpara,net_dg2.netpara,net_dg3.netpara])

    update_h = tf.train.AdamOptimizer(lr[3]).minimize(loss_dg, var_list=h_list)
    g1 = net_dg1.get_g(h_input)
    g2 = net_dg2.get_g(h_input)
    g3 = net_dg3.get_g(h_input)
    ...
    for k in range(epochs[0]):
        X1, X2, X3,gt = shuffle(X1, X2, X3,gt)
        for batch_x1, batch_x2, batch_x3, batch_No in next_batch(X1, X2,X3, batch_size):
            _, val_pre = sess.run([pre_train, loss_pre], feed_dict={x1_input: batch_x1, x2_input: batch_x2,x3_input: batch_x3})
    ...
    for j in range(epochs[1]):
        X1, X2,X3, H, gt = shuffle(X1, X2,X3, H, gt)
        for num_batch_i in range(int(num_batchs) - 1):
            ...
            batch_g3 = sess.run(g3, feed_dict={h_input: batch_h})

            # ADM-step1: optimize inner AEs and
            _, val_ae = sess.run([update_ae, loss_ae], feed_dict={x1_input: batch_x1, x2_input: batch_x2,x3_input: batch_x3,
                                                                  fea1_latent: batch_g1, fea2_latent: batch_g2, fea3_latent: batch_g3})
            ...
            batch_z_half3 = sess.run(z_half3, feed_dict={x3_input: batch_x3})
            ...
            _, val_dg = sess.run([update_dg, loss_dg], feed_dict={fea1_latent: batch_z_half1,
                                                                  ...
                                                                  fea3_latent: batch_z_half3})

            # ADM-step3: update H
            for k in range(epochs[2]):
                sess.run(update_h, feed_dict={fea1_latent: batch_z_half1, fea2_latent: batch_z_half2,fea3_latent: batch_z_half3})
            ...
            val_total = sess.run(loss_ae, feed_dict={x1_input: batch_x1, x2_input: batch_x2,x3_input: batch_x3,
                                                     fea1_latent: batch_g1_new, fea2_latent: batch_g2_new,fea3_latent: batch_g3_new})
